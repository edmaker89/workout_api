...
async def post(db_session: DatabaseDependency, 
               centro_treinamneto_in: CentroTreinamentoIn = Body(...)
               ) -> CentroTreinamentoOut:
    
    centro_treinamento_out = CentroTreinamentoOut(id=uuid4() ,**centro_treinamneto_in.model_dump())
...
@router.get('/', summary='Constular todos os centros de treinamento', 
...
@router.get('/{id}', summary='Constular um centro de treinamento pelo id', 
             status_code=status.HTTP_200_OK,
             response_model=CentroTreinamentoOut,
             )
async def query(id:UUID4 ,db_session: DatabaseDependency) -> CentroTreinamentoOut:
    ...
    if not centro_treinamento:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Centro treinamento não encontrada no id: {id}')
